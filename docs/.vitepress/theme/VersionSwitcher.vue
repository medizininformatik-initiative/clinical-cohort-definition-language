...
function detectCurrentVersion(): string {
  ...
  // Path format: /clinical-cohort-definition-language/dev/, /clinical-cohort-definition-language/stable/, or /clinical-cohort-definition-language/v0.4.0/...
  const match = window.location.pathname.match(/^\/clinical-cohort-definition-language\/(v[\d.]+|dev|stable)/)
...
onMounted(async () => {
  ...
  try {
    const res = await fetch('/clinical-cohort-definition-language/versions.json')
    if (res.ok) {
